    ...
    <div style={containerStyle} className="text-white p-14 flex flex-col">
      ...
      <div className="flex justify-between items-start mb-14 relative z-10">
        <div className="flex items-center gap-6">
            ...
            <div>
                <p className="text-[9px] font-black uppercase tracking-[0.6em] text-gray-500">Official Dispatch</p>
                <p className="text-[7px] font-bold uppercase tracking-[0.4em] text-red-600 mt-1">Global Infrastructure</p>
            </div>
        </div>
        <div className="text-right">
          <h1 className="text-8xl font-black tracking-tighter leading-[0.75] italic text-white">TOP 10<br/><span className="text-red-600">TODAY.</span></h1>
          <p className="text-xl font-bold text-gray-700 uppercase tracking-[0.4em] mt-4">{date}</p>
        </div>
      </div>

      {/* List - Adjusted for 10 items */}
      <div className="flex-grow space-y-3.5 relative z-10">
        {topFilms.map((film, index) => (
          <div key={film.key} className="flex items-center gap-8 bg-white/[0.02] border border-white/5 p-3.5 rounded-[2rem] shadow-2xl overflow-hidden relative group">
            ...
            <div className="relative w-24 h-32 flex-shrink-0 rounded-xl overflow-hidden shadow-[0_15px_30px_rgba(0,0,0,0.8)] border border-white/10">
                <img 
