...
export async function POST(request: Request) {
  try {
    ...
    // Determine amount and note based on payment type, using server-side validation.
    if (paymentType === 'donation' || paymentType === 'billSavingsDeposit' || paymentType === 'crateFestPass') {
        ...
        amountInCents = priceMap[paymentType]; // Use server price, ignore client-sent amount
        switch(paymentType) {
            case 'subscription':
