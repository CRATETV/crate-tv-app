...
export async function POST(request: Request) {
  try {
    ...
    await resend.emails.send({
      ...
      to: recipientEmail,
      // FIX: Corrected the `reply_to` property to `replyTo` in the `resend.emails.send` call to match the Resend library's expected type, resolving a TypeScript error.
      reply_to: data.email,
