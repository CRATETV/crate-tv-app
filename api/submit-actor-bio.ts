...
const safeUrl = (v: unknown) => { const s = String(v ?? '').trim(); return /^https?:\/\//i.test(s) ? escHtml(s) : ''; };
...
export async function POST(request: Request) {
  try {
    if (!rateLimit(`actor-bio:${getIP(request)}`, 5, 60 * 60_000)) {
      return new Response(JSON.stringify({ error: 'Too many submissions. Please try again later.' }), { status: 429, headers: { 'Content-Type': 'application/json' } });
    }
    ...
    const bodyHtml = `
      ...
      <h1 style="margin:0 0 20px;font-size:24px;font-weight:900;text-transform:uppercase;">Actor Profile: ${escHtml(actorName)}</h1>
      <p style="margin:0 0 8px;">A new actor profile is awaiting review.</p>
      <p style="margin:0 0 24px;"><strong>Email:</strong> ${escHtml(email)}</p>
      ${renderEmailButton('Review in Admin → Submissions', 'https://cratetv.net/admin')}
    `;
    ...
    await resend.emails.send({
      ...
      html: renderBrandedEmail({ title: `New Actor Submission: ${escHtml(actorName)}`, bodyHtml }),
    });
