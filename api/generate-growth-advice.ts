...
export async function POST(request: Request) {
  try {
    ...
    try {
        ...
        if (aiError.message?.includes('RESOURCE_EXHAUSTED') || aiError.isQuotaError) {
             return new Response(JSON.stringify({ 
                error: "The AI Strategy Consultant is busy right now (Daily Quota Hit). Please check back in 24 hours.",
                isQuotaError: true 
