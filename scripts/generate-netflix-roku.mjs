...
// 6. ACCOUNT LINK SCENE (Login)
// Fixed colors: White text, Red accents. No Blue.
...
<component name="HomeScene" extends="Group">
    ...
    <children>
        ...
        <Group id="heroGroup" translation="[100, 250]">
            <!-- Removed "NOW STREAMING" Label as requested -->
            ...
            <LayoutGroup layoutDirection="horiz" itemSpacings="[20]" translation="[0, 130]">
                <Label id="heroMeta" color="#CCC"><Font role="font" uri="pkg:/fonts/Regular.ttf" size="28" /></Label>
...
// Fixed Playback Logic in HomeScene.brs
...
sub onSelect()
    ' FIX: Get the exact item clicked from the row list
    row = m.rowList.rowItemFocused[0]
    col = m.rowList.rowItemFocused[1]
    content = m.rowList.content
...
// 8. PLAYER OVERLAY (With Actor Bio)
...
sub onContentSet()
    ...
    if item.actors <> invalid
        for each actor in item.actors
            ' Formatting: Name (Bio snippet)
            actors = actors + "• " + actor.name + chr(10) + "  " + actor.bio + chr(10) + chr(10)
        end for
    end if
...
console.log("\n---------------------------------------------------");
console.log("✅ PREMIUM ROKU APP GENERATED");
