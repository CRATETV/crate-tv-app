...
// 5. SIDEBAR COMPONENT (Animated Modal)
...
<component name="SideBar" extends="Group">
    ...
    <children>
        ...
        <Rectangle id="overlay" width="1920" height="1080" color="#000000" opacity="0.0" visible="false" />
        
        <!-- Menu Drawer (Starts off-screen) -->
        <Group id="drawer" translation="[-400, 0]">
            <Rectangle width="400" height="1080" color="#141414" opacity="0.98" />
            
            <!-- Logo area -->
            <Poster uri="pkg:/images/logo_hd.png" width="150" height="80" translation="[50, 50]" />

            <!-- Menu Items -->
            <LayoutGroup id="menu" translation="[40, 200]" itemSpacings="[20]">
                <!-- Using standard Buttons but styled later via focus -->
                <Button id="navSearch" text="  Search" minWidth="300" height="60" iconUri="" showFocusFootprint="true" />
                <Button id="navHome" text="  Home" minWidth="300" height="60" iconUri="" showFocusFootprint="true" />
                <Button id="navClassics" text="  Classics" minWidth="300" height="60" iconUri="" showFocusFootprint="true" />
                <Button id="navPortals" text="  Portals" minWidth="300" height="60" iconUri="" showFocusFootprint="true" />
                <Button id="navAccount" text="  Account" minWidth="300" height="60" iconUri="" showFocusFootprint="true" />
            </LayoutGroup>
        </Group>

        <!-- Animations -->
        <Animation id="slideIn" duration="0.25" easeFunction="outCubic">
            <Vector2DFieldInterpolator key="[0.0, 1.0]" keyValue="[ [-400, 0], [0, 0] ]" fieldToInterp="drawer.translation" />
            <FloatFieldInterpolator key="[0.0, 1.0]" keyValue="[ 0.0, 0.7 ]" fieldToInterp="overlay.opacity" />
        </Animation>
        
        <Animation id="slideOut" duration="0.2" easeFunction="outCubic">
            <Vector2DFieldInterpolator key="[0.0, 1.0]" keyValue="[ [0, 0], [-400, 0] ]" fieldToInterp="drawer.translation" />
            <FloatFieldInterpolator key="[0.0, 1.0]" keyValue="[ 0.7, 0.0 ]" fieldToInterp="overlay.opacity" />
        </Animation>
...
sub onExpandedChange()
    if m.top.expanded
        ...
        ' Focus first item or remembered item
        m.navButtons[1].setFocus(true) 
    else
        m.slideIn.control = "stop"
        m.slideOut.control = "start"
        ' When slide out finishes, we could hide overlay, but timeline handles opacity
    end if
end sub

sub onSelect(evt)
    btn = evt.getRoSGNode()
    m.top.itemSelected = { id: btn.id }
end sub

function onKeyEvent(key, press)
    if press
        if key = "right"
            ' Close menu, return to content
            return false 
        end if
    end if
    return false
...
<component name="MainScene" extends="Scene">
    ...
    <children>
        ...
        <SideBar id="sideBar" visible="true" />
...
sub navigateTo(componentName, args = invalid)
    ...
    if componentName = "DetailsView" or componentName = "AccountLinkScene"
        ...
        m.sideBar.expanded = false ' Always collapse on new page load
    end if
end sub
...
sub onNavSelect(evt)
    ...
    else if id = "navClassics"
        navigateTo("ClassicsView")
...
function onKeyEvent(key, press)
    if press
        if key = "left"
            ' If Sidebar is present but collapsed, and user hits left edge, OPEN IT
            if m.sideBar.visible and not m.sideBar.expanded
                ' We assume views handle internal nav. If view didn't consume "left", we open sidebar.
                m.sideBar.expanded = true
                m.sideBar.setFocus(true)
                return true
            end if
        else if key = "right"
            if m.sideBar.visible and m.sideBar.expanded
                m.sideBar.expanded = false
...
// 7. HOME VIEW (Cinematic, Dark, Red Accents)
...
<component name="HomeView" extends="Group">
    ...
    <children>
        ...
        <Rectangle width="1920" height="1080" color="#141414" opacity="0.6" />
        ...
        <Group id="heroGroup" translation="[120, 250]">
            ...
            <Group translation="[0, 320]">
                <Rectangle width="250" height="70" color="#E50914" cornerRadius="5"><Label text="PLAY" width="250" height="70" horizAlign="center" vertAlign="center" color="#FFF"><Font role="font" uri="pkg:/fonts/Bold.ttf" size="30" /></Label></Rectangle>
            </Group>
        </Group>
        ...
        <RowList 
            ...
            numRows="2" 
            rowItemSize="[[360, 203]]" 
            rowItemSpacing="[[30, 60]]" 
            itemSize="[1920, 350]" 
            rowLabelOffset="[[120, -30]]" 
...
function onKeyEvent(key, press)
    if press
        if key = "left"
            ' If on first column, bubble up to MainScene to open Menu
            if m.rowList.rowItemFocused[1] = 0
                return false ' MainScene handles this
            end if
        end if
    end if
    return false
...
// 8. CINEMATIC POSTER (Red Glow Focus)
...
<component name="CinematicPosterItem" extends="Group">
    ...
    <children>
        ...
        <!-- RED FOCUS RING -->
        <Rectangle id="focusRing" width="376" height="219" translation="[-8, -8]" color="#E50914" fill="false" opacity="0" stroke="6" />
    </children>
</component>
`);
...
// 10. SEARCH & CLASSICS VIEWS (Placeholders for now to prevent errors)
writeFile('components/SearchView.xml', `<?xml version="1.0" encoding="utf-8" ?><component name="SearchView" extends="Group"><children><Rectangle width="1920" height="1080" color="#141414" /><Label text="Search Coming Soon" translation="[100,100]" /></children></component>`);
writeFile('components/SearchView.brs', `sub init() : end sub`);
writeFile('components/ClassicsView.xml', `<?xml version="1.0" encoding="utf-8" ?><component name="ClassicsView" extends="Group"><children><Rectangle width="1920" height="1080" color="#141414" /><Label text="Classics Coming Soon" translation="[100,100]" /></children></component>`);
writeFile('components/ClassicsView.brs', `sub init() : end sub`);

// 11. API TASK & ACCOUNT LINK (Reused from V9 stable)
...
console.log("---------------------------------------------------");
console.log("✅ ROKU V10 (CINEMATIC IMMERSION) GENERATED");
