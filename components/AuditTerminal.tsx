...
const AuditTerminal: React.FC = () => {
    ...
    const handleTestLog = async () => {
        ...
        try {
            const res = await fetch('/api/log-audit-event', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ password: pass, operatorName: name, action: 'TEST_HEARTBEAT', type: 'SECURITY', details: 'Manual audit stream verification triggered by operator.' }),
            });
            if (!res.ok) throw new Error('Failed to log test event.');
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-white/5 pb-8">
                <div>
                    ...
                    <p className="text-gray-500 text-xs font-bold uppercase tracking-widest mt-2">
                        Immutable modification history — auto-refreshes every 15s
                ...
                <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <button onClick={handleTestLog} className="bg-red-600/10 hover:bg-red-600/20 text-red-500 font-black py-2.5 px-6 rounded-xl transition-all uppercase text-[10px] tracking-widest border border-red-600/30">Test Audit</button>
                    ...
                    <button onClick={handleExportCSV} className="bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white font-black py-2.5 px-6 rounded-xl transition-all uppercase text-[10px] tracking-widest border border-white/10">Export (.csv)</button>
                    <div className="flex items-center gap-3 text-[10px] font-black text-gray-700 uppercase tracking-widest">
                        <span className={`w-2 h-2 rounded-full ${logs.length > 0 ? 'bg-green-500 animate-pulse' : 'bg-amber-500'}`}></span>
                        {logs.length > 0 ? `${filteredLogs.length} of ${logs.length} Records` : 'No Data'}
                    </div>
                </div>
            </div>
            ...
            <div className="bg-black border border-white/10 rounded-3xl overflow-hidden shadow-[inset_0_0_50px_rgba(0,0,0,1)]">
                {filteredLogs.length === 0 ? (
                    <div className="p-20 text-center space-y-4">
                        <p className="text-gray-600 font-black uppercase tracking-[0.3em] text-sm">{searchQuery ? 'No Matching Records' : 'No Audit Records Found'}</p>
                        <p className="text-gray-700 text-xs italic">{searchQuery ? `No records match "${searchQuery}".` : 'Perform an administrative action to generate a trace.'}</p>
                    </div>
                ) : (
                            ...
                            <tbody className="divide-y divide-white/5">
                                {filteredLogs.map(log => (
                                    <tr key={log.id} className="hover:bg-white/[0.02] transition-colors">
                                        ...
                                        <td className="p-5">
                                            <span className="text-green-500 bg-green-900/10 border border-green-900/30 px-2 py-0.5 rounded-[4px] font-black text-[9px]">LOGGED</span>
                                        </td>
                                        <td className="p-5 text-gray-600 font-mono text-[10px]">{log.ipAddress || log.ip || '---'}</td>
