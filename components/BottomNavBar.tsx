...
const BottomNavBar: React.FC<BottomNavBarProps> = ({ onSearchClick }) => {
    const { user } = useAuth();
    const { isFestivalLive, settings } = useFestival();
    ...
    return (
        ...
        >
            ...
            <NavItem 
                path="/library" 
                activePath={activePath} 
                icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>}
                label="Library" 
            />
            <NavItem 
                path="/search" 
            ...
            />
            {settings?.pwffProgramVisible && (
                <NavItem 
                    path={`/pwff${settings?.pwffUrlYear || '2026'}`}
                    activePath={activePath}
