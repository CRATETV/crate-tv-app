...
const MovieCard: React.FC<MovieCardProps> = ({ movie, onSelectMovie, rank }) => {
  ...
  if (rank) {
    return (
      ...
      >
        <div 
          className="text-[8rem] sm:text-[10rem] md:text-[12rem] font-black text-white transition-transform duration-300 ease-in-out group-hover:scale-110"
          style={{ textShadow: '0 2px 8px rgba(0,0,0,0.7)', WebkitTextStroke: '3px #141414', paintOrder: 'stroke fill' }}
          aria-hidden="true"
        >
          {rank}
        </div>
        <div className="relative w-28 sm:w-32 md:w-40 aspect-[3/4] rounded-md overflow-hidden bg-gray-900 transition-transform duration-300 ease-in-out group-hover:scale-110 -ml-10 sm:-ml-12 md:-ml-16 shadow-lg">
          {posterContent}
        </div>
      </div>
    );
