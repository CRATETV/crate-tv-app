...
const FestivalEditor: React.FC<FestivalEditorProps> = ({ data, config, allMovies, onDataChange, onConfigChange, onSave, isSaving }) => {
  ...
  const removeDay = (index: number) => {
      if (!window.confirm("PURGE DAY: Permanently erase this session?")) return;
    ...
    <div className="space-y-12 pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="flex items-center gap-4">
             ...
             <button 
                ...
                className={`font-black py-4 px-12 rounded-2xl uppercase tracking-widest text-xs shadow-2xl transition-all active:scale-95 ${isDirty ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-gray-800 text-gray-600 cursor-not-allowed border border-white/5'}`}
            >
                {isSaving ? 'Syncing...' : 'Push Global Manifest'}
            </button>
        </div>
      </div>
      ...
      <div className="bg-[#0f0f0f] p-10 rounded-[3rem] border border-white/5 space-y-10 shadow-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                  <label className="form-label">Primary Headline</label>
              ...
              <div>
                  <label className="form-label">Subheader / Season Title</label>
                  <input type="text" name="subheader" value={config.subheader || ''} onChange={handleConfigChange} placeholder="e.g. 12th Annual Official Selections" className="form-input bg-black/40" />
              </div>
              <div className="md:col-span-2">
                  <label className="form-label">Mission Narrative</label>
                ...
                <div key={blockIndex} className="bg-black/60 p-8 rounded-3xl border border-white/10 flex flex-col md:flex-row justify-between items-center gap-10 group shadow-inner">
                   <div className="flex-grow space-y-4 w-full md:w-auto">
                        ...
                        <div className="flex gap-12 items-center">
                            <div className="space-y-1">
                                <label className="text-[8px] text-gray-700 font-black tracking-widest uppercase">Uplink Time</label>
