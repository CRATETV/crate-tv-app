...
export const WatchPartyPage: React.FC<WatchPartyPageProps> = ({ movieKey }) => {
    ...
    const handleShare = async () => {
        const shareUrl = `${window.location.origin}/watchparty/${movieKey}`;
        const shareText = `Join me for a live Watch Party of "${movie?.title}" on Crate TV! 🎬✨\n\n(No account? Join for free to watch with us!)`;

        if (navigator.share) {
            try {
                await navigator.share({
                    title: `Watch Party: ${movie?.title}`,
                    text: shareText,
                    url: shareUrl,
                });
            } catch (err) {
                ...
                <div className="flex-grow flex flex-col relative h-full min-h-0">
                    ...
                    <div className="flex-none bg-black/90 backdrop-blur-md p-3 flex items-center justify-between border-b border-white/5 pt-[max(0.75rem,env(safe-area-inset-top))]">
                        ...
                        <div className="relative">
                            <button 
                                onClick={handleShare}
                                className="flex items-center gap-2 bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-lg border border-white/10 transition-all text-[10px] font-black uppercase tracking-widest"
                            >
