    ...
    <div style={containerStyle} className="text-white p-12 flex flex-col">
      ...
      <div className="flex justify-between items-start mb-8 relative z-10">
        ...
        <div className="text-right">
          <h1 className="text-6xl font-black tracking-tighter leading-[0.75] italic text-white">TOP 10<br/><span className="text-red-600">TODAY.</span></h1>
          <p className="text-base font-bold text-gray-700 uppercase tracking-[0.4em] mt-3">{date}</p>
        </div>
      </div>

      {/* List - Refined to prevent title truncation */}
      <div className="flex-grow space-y-2 relative z-10">
        {topFilms.map((film, index) => (
          <div key={film.key} className="flex items-center gap-6 bg-white/[0.02] border border-white/5 p-2 rounded-[2rem] shadow-2xl overflow-hidden relative group h-[155px]">
            ...
            <div className="relative w-18 h-28 flex-shrink-0 rounded-xl overflow-hidden shadow-2xl border border-white/10">
                <img 
            ...
            <div className="flex-grow min-w-0 relative z-10 pr-6">
                <h2 className="text-3xl font-black uppercase tracking-tighter italic leading-[1.1] mb-2 line-clamp-2">{film.title}</h2>
