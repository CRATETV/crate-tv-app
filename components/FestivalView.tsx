...
const FestivalView: React.FC<FestivalViewProps> = ({ festivalData, festivalConfig, allMovies, showHero = true }) => {
  ...
  const handlePaymentSuccess = () => {
    // Handle post-payment logic, e.g., show a confirmation message
      ...
      {showHero && (
        <div className="relative bg-gradient-to-br from-[#2c1a4d] via-[#1a2c4d] to-[#141414] text-center py-12 sm:py-20 px-4 overflow-hidden">
          ...
          <div className="relative z-10 max-w-4xl mx-auto">
              <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white mb-4 tracking-tight" style={{ textShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>{festivalConfig.title}</h1>
              <p className="text-base sm:text-lg md:text-xl text-gray-300 max-w-2xl mx-auto mb-8">
                  {festivalConfig.description}
              </p>
              <div 
                className="inline-block bg-gradient-to-r from-yellow-400 to-amber-500 text-black font-bold py-3 px-6 sm:py-4 sm:px-8 rounded-lg text-lg sm:text-xl shadow-lg transform transition-transform hover:scale-105 cursor-pointer"
                              ...
                              <div key={block.id} className="bg-gradient-to-br from-gray-900 to-[#101010] border border-gray-800 rounded-xl shadow-lg overflow-hidden">
                                <div className="p-6 bg-black/20">
                                   <div className="flex flex-col sm:flex-row justify-between sm:items-center">
                                     <div>
                                      <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2">{block.title}</h2>
                                      <p className="font-semibold text-purple-300">{block.time}</p>
                                    </div>
                                    <div 
                                      className="flex-shrink-0 bg-teal-500 hover:bg-teal-600 text-white font-bold py-2 px-4 rounded-lg transition-transform hover:scale-105 cursor-pointer shadow-md text-sm"
