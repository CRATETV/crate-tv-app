...
const PauseOverlay: React.FC<PauseOverlayProps> = ({ 
    ...
    return (
        <div 
            ...
            onClick={onResume}
        >
            {/* Top Left: Home Button (Always Visible) */}
            <div className="w-full flex justify-between items-start" onClick={stopPropagation}>
                 <button 
                    onClick={onHome} 
                    className="flex items-center gap-2 text-white bg-black/60 hover:bg-black/80 px-4 py-2 rounded-full backdrop-blur-md transition-colors border border-white/20 z-50"
            ...
            {/* Center Content */}
            <div className="flex flex-col items-center justify-center w-full flex-grow" onClick={stopPropagation}>
                {!isMobile && (
                ...
                <div className="flex items-center justify-center gap-8 md:gap-12">
                    <button onClick={onRewind} className="p-3 hover:scale-110 transition-transform" aria-label="Rewind 10 seconds">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 md:h-12 md:w-12 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 19l-7-7 7-7" /></svg>
                    </button>

                    <button onClick={onResume} className="transform hover:scale-110 transition-transform" aria-label="Resume playback">
                        <div className="bg-white/10 hover:bg-white/20 rounded-full p-4 md:p-6 border-2 border-white/30">
                           <svg xmlns="http://www.w3.org/2000/svg" className="h-12 w-12 md:h-16 md:w-16 text-white" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" /></svg>
                        </div>
                    </button>

                     <button onClick={onForward} className="p-3 hover:scale-110 transition-transform" aria-label="Forward 10 seconds">
                       <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 md:h-12 md:w-12 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 5l7 7-7 7" /></svg>
                    </button>
                </div>
            </div>
            
            {/* Bottom Action Buttons */}
            <div className="w-full flex items-center justify-around md:justify-center md:gap-10 pb-8" onClick={stopPropagation}>
                {isMobile ? (
                    // Mobile Buttons
                    <>
