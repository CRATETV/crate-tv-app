...
const Header: React.FC<HeaderProps> = ({ searchQuery, onSearch, isScrolled, onMobileSearchClick, onSearchSubmit }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-40 p-4 flex justify-end items-center transition-colors duration-500 ${isScrolled ? 'bg-[#141414]' : 'bg-gradient-to-b from-black/70 to-transparent'}`}>
      {/* Desktop Search */}
      <form onSubmit={handleSearchSubmit} className="relative w-full max-w-[200px] sm:max-w-xs hidden md:block">
        <input
          type="text"
          name="search"
          value={searchQuery}
