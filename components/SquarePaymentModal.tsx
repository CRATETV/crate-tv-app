    ...
    const renderContent = () => {
        ...
        if (!isDonation) {
             return (
                <div className="text-center p-8">
                    ...
                    <p className="text-gray-300">This feature is not yet available. Please check back later.</p>
                </div>
            );
        }
        ...
        return (
            <form onSubmit={handlePayment}>
                <h2 className="text-2xl font-bold text-white mb-2">Support "{movie?.title}"</h2>
                <p className="text-sm text-gray-400 mb-6">Your contribution directly supports the filmmakers.</p>
                ...
                <button type="submit" className="submit-btn w-full" disabled={status === 'processing' || status === 'loading'}>
                    {status === 'processing' ? 'Processing...' : `Contribute $${amount}`}
                </button>
